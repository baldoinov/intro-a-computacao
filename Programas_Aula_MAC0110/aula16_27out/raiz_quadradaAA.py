...
def raiz_quadrada(x, eps):
    ...
    while math.fabs(atual - anterior) >= eps:   #<= Trocar para comparar com o valor dado pela funcao sqrt() do Python
                                                # Nova comparacao ficaria assim
        anterior = atual
        atual = (anterior + x / anterior) / 2
        n = n + 1 
        print("valor atual = ", atual)  # para ver os valores de r2, r3, ...
        
    return atual, n  #<======== Veja! Para retornar mais de um valor, separar esses valores por vírgulas
